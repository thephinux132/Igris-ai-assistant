...
import readline
cli_history_file = Path.home() / "OneDrive" / "Documents" / "ai_script_history" / "igris_cli_memory.json"
cli_history_file.parent.mkdir(parents=True, exist_ok=True)
if not cli_history_file.exists():
    cli_history_file.write_text("[]")

def save_cli_memory(entry):
    try:
        import json
        mem = json.loads(cli_history_file.read_text(encoding="utf-8"))
        mem.append(entry)
        if len(mem) > 50:
            mem = mem[-50:]
        cli_history_file.write_text(json.dumps(mem, indent=2))
    except:
        pass
...
def load_cli_memory():
    try:
        import json
        return json.loads(cli_history_file.read_text(encoding="utf-8"))
    except:
        return []

def handle_cli_command(command, cfg, policy):
    command = command.strip()
    if command == "/history":
        mem = load_cli_memory()
        if not mem:
            return "[History] No previous entries found."
        return "\n".join([f"{i+1}. {m['input']} → {m['output']}" for i, m in enumerate(mem[-10:])])
    if command == "/help":
        tags = {}
        for t in cfg.get("tasks", []):
            for tag in t.get("tags", []):
                tags.setdefault(tag, []).append(t["task"])
        out = "[Help Menu]\n"
        for tag, items in tags.items():
            out += f"\n[{tag.upper()}]\n" + "\n".join(f" - {x}" for x in items)
        return out
    if command.startswith("/auto "):
        val = command.split(" ", 1)[-1].strip().lower()
        if val in ("on", "off"):
            policy["auto_execute"] = (val == "on")
            return f"Auto-execution set to {val.upper()}."
        else:
            return "Usage: /auto on | /auto off"
    if command.startswith("/learn"):
        try:
            raw = input("Enter new task name: ").strip()
            phrases = input("Comma-separated trigger phrases: ").strip().split(",")
            action = input("Shell command or plugin: ").strip()
            is_plugin = action.startswith("plugin:")
            requires_admin = input("Requires admin? (y/N): ").strip().lower().startswith("y")
            ...
            return f"✅ Learned new task: {raw}"
        except Exception as e:
            return f"[ERROR] Failed to learn task: {e}"
    return "[INFO] Unknown command. Available: /learn, /history, /auto on|off, /help"
...
def main():
    ...
    try:
        ...
        while True:
            ...
            save_cli_memory({ "input": user_req, "output": output })
            if user_req.startswith("/"):
                output = handle_cli_command(user_req, cfg, policy)
            print(output + "\n")
