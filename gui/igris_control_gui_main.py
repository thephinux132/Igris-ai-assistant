...
class App(tk.Tk):
    ...
    def handle_response(self, resp, user_req):
        ...
        try:
            data = self._extract_json_from_response(resp)  # Parses AI response into JSON
            if data is None:
 ...
 # Check if the task requires admin rights or is in the enforced list.
    ...
    def _extract_json_from_response(self, resp):
        ...
        try:
            ...
            # Defensive: make sure expected keys exist
            for key in ["task_name", "action", "requires_admin"]:
                if key not in parsed:
                    raise KeyError(f"Missing key in AI response: '{key}'")
            return parsed
            ...
            if fallback:
                ...
                return None
            # If all else fails, try local fallback match from task_intents
            ...
            fallback = find_best_local_match(user_req, load_config("task_intents.json"))
            if fallback:
                self._handle_matched_task({"task_name": fallback["task"], "action": fallback["action"], "requires_admin": fallback.get("requires_admin", False), "reasoning": "Matched locally from trained intents"}, user_req)
                return
            else:
                raise # Re-raise exception if no fallback
