...
try:
    from igris_core import strict_json_from_text, ask_ollama as core_ask, verify_admin_pin
except ModuleNotFoundError:
    from core.igris_core import strict_json_from_text, ask_ollama as core_ask, verify_admin_pin
...
def load_aliases():
    ...
    try:
        ...
        print(f"[WARN] Could not parse aliases file {ALIASES_FILE}: {e}")
    except Exception as e:
        # Catch other potential I/O errors
        print(f"[WARN] Failed to load aliases from {ALIASES_FILE}: {e}")
    return {}
...
def prompt_for_pin(root, pin_hash):
    """
    Display a modal dialog prompting the user for their admin PIN.  Uses
    `verify_admin_pin()` from igris_core to perform a constant‑time check
    against the stored hash.  Returns True if the entered PIN matches,
    otherwise False.
    """
    confirmed = []
    ...
    def submit_pin():
        ...
        # Use the core helper for constant‑time comparison against the stored hash
        if verify_admin_pin(entered, {"admin_pin_hash": pin_hash}):
            confirmed.append(True)
            top.destroy()
        else:
...
class App(tk.Tk):
 ...
 # Check if the task requires admin rights or is in the enforced list.
    ...
    def local_match_action(self, user_req: str) -> dict | None:
    ...
    def send_request(self):
        try:
            ...
            lm = self.local_match_action(user_req)
            if lm:
