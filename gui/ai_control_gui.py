...
class App(tk.Tk):
    ...
    def show_plugin_menu(self):
        # Placeholder plugin menu
        from tkinter import messagebox
        messagebox.showinfo("Plugins", "Plugin menu not yet implemented.")
    ...
    def build_ui(self):
        ...
        # --- Menu Bar ---
        self.auto_execute = tk.BooleanVar(value=policy.get("auto_execute_default", False))
        self.tts_enabled_var = tk.BooleanVar(value=policy.get("tts_enabled", False))
    ...
    def send_request(self):
        try:
            ...
            review = self.respond_with_review(user_req)
            if review:
            ...
            def worker():
                ...
                prompt = f"{context_str}{base_context}\n\nUser request: {user_req}"
...
def get_system_uptime():
    ...
    def run_startup_tasks(self):
        ...
        if username:
            ...
            if self.tts_enabled_var.get():
             speak(greeting)
            if policy.get("daily_checkup_enabled", True): # Enabled by Default
                self.after(1000, self.run_daily_checkup)
        else:
            self.destroy()
            sys.exit("Username is required to proceed.")
    ...
    def _handle_matched_task(self, data, user_req):
        ...
        if requires_admin or task_name in enforce_on_tasks:
            ...
            if not self.show_fingerprint_prompt():
        ...
        if self.auto_execute.get():
            threading.Thread(target=self._execute_task, args=(action,), daemon=True).start()
        else:
            self.chat_area.insert(tk.END, "[INFO] Auto-execute is off. Command not run.\n\n")

    def _extract_json_from_response(self, resp):
        """Extracts and loads JSON from AI response, handling missing JSON objects and errors."""
        try:
            json_match = re.search(r'\{.*\}', resp, re.DOTALL)
            if not json_match:
                raise json.JSONDecodeError("No JSON object found in response", resp, 0)
            return json.loads(json_match.group(0))
        except json.JSONDecodeError as e:
            self._log_message("Response Error", f"JSON parsing error: {e}\nResponse: {resp}")
            # Use fallback behavior if available
            fallback = identity.get("fallback_behavior", {}).get("on_no_match")
            if fallback:
                self._log_message("Fallback", f"Using fallback behavior: {fallback}")
                self.chat_area.insert(tk.END, f"AI: {fallback}\n\n")
                return None  # Indicate fallback was used
            else:
                raise  # Re-raise exception if no fallback

    def handle_response(self, resp, user_req):
        """Handles AI responses, extracting actions, and managing fallback behaviors."""
        try:
            data = self._extract_json_from_response(resp) # Use the updated JSON extraction method
            if data is None:  # Fallback was used
                return
